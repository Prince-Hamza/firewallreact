...
useEffect(() => {
    productDetail&&productDetail?.attributes?.forEach((item)=>{
        if (item.name === "Html Specs")  setHtmlString(item&&item?.options[0])
    })
        ...
        <div className="pt-5">
            ...
            <section className="mt-5">
                ...
                <div class="justify px-4 " style={{ maxHeight: "25rem", overflow: "auto" }} className={active === 1 ? "d-none d-sm-block" : "d-none"}>
                        ...
                        { ReactHtmlParser(htmlString) ? ReactHtmlParser(htmlString) : ReactHtmlParser(productDetail && productDetail.description) }
                        </div>
                        <div class="justify shortDes px-1" style={{  overflow: "auto" }} className={active === 2 ? "d-none d-sm-block" : "d-none"}>
                            <span style={{ fontSize: "small", color:'#575757', fontFamily:'Poppins' }}>{ReactHtmlParser(productDetail && productDetail.short_description)}</span>
